...
Use MermaidJS (` ```mermaid `) for all workflow and architecture diagrams. This ensures consistent rendering across GitHub, VS Code, and documentation tools.

- **Flowcharts** (`flowchart TD`) for sequential processes and decision flows
- **Sequence diagrams** (`sequenceDiagram`) for multi-actor interactions
- **Class diagrams** (`classDiagram`) for data structures and relationships

Do **not** convert directory trees or terminal output templates to MermaidJS — keep those as ASCII.
